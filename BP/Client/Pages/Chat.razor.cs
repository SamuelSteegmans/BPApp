...
namespace BP.Client.Pages
{
    public partial class Chat
    {
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            hubConnection.On<ChatMessage, string>("ReceiveMessage", async (message, userName) =>
            {
                if ((ContactId == message.ToUserId && CurrentUserId == message.FromUserId) || (ContactId == message.FromUserId && CurrentUserId == message.ToUserId))
                {
                    ...
                    {
                        messages.Add(new ChatMessage { Message = message.Message, CreatedDate = message.CreatedDate, FromUser = new ApplicationUser() { Email = CurrentUserEmail, UserName = userName } });
                    ...
                    {
                        messages.Add(new ChatMessage { Message = message.Message, CreatedDate = message.CreatedDate, FromUser = new ApplicationUser() { Email = ContactEmail, UserName = ContactUserName } });
                    }
        ...
        [Parameter] public string ContactUserName { get; set; }
        ...
        private async Task GetLikedUsersAsync()
        {
            ContactId = null;
            CurrentMessage = string.Empty;
            messages = new List<ChatMessage>();
            ChatUsers = await _chatManager.GetLikedUsersAsync();
        }
